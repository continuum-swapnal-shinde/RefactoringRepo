...
//			strOutput = getCommandOutput(objBufferReader);
		...
		case "LinuxEndpointID":
			CustomWait.sleep(30);
			strOutput = getConsoleOutput(objBufferReader);
			break;
			
		case "CurlExecution":
			 StringBuilder outputResult1 = new StringBuilder();
		        while (true) {
		            try {
						line = objBufferReader.readLine();
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
		            if (line == null) { break; }
		            outputResult1.append(line + "\n");
		            System.out.println(line);
		        }
		       return outputResult1.toString();
	...
	/**
	 * This method is used to copy files from remote machine to localmachine
	 */
	public void copyFileFromRemote() {
		System.out.println("This is the download file location");
		...
		try {
			//this.cs.cd("platform-agent-core.exe.zip");
			this.cs.get("dev_platform-agent-core.zip","dev_platform-agent-core.zip");
			System.out.println("File successfully copied from remote.");
		} catch (SftpException e) {
			System.out.println("Exception occurred while file download.");
			e.printStackTrace();
		} 
	}
