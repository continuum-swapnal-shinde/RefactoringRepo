...
//		}
	...
	@When("^User send a Post request for package mapping API as \"([^\"]*)\"$")
	public void I_send_a_Post_request(String endpoint) throws Throwable {
		try {
			String url="";
			CommonSteps.fetchTestData();
			url = Utilities.getMavenProperties("Agent-Service-ELB") + endpoint + data.get(index).get("feature");
			GlobalVariables.scenario.write("\n\n Sending a Post request for URL " + url);
			CommonSteps.fetchTestData();
			GlobalVariables.packageToFeatureJson = ems.packageFeatureMapping(GlobalVariables.data, GlobalVariables.index);
			GlobalVariables.scenario.write("\n\nJson format used for post :" + GlobalVariables.packageToFeatureJson);
			GlobalVariables.conn = HTTPUtility.sendPostReqWithPayload(url, GlobalVariables.packageToFeatureJson);
		} catch (Exception e) {
			ErrorReporter.reportError(e);
		}
	}
	...
	public void I_validate_ITSPlatform_version() throws Throwable {
		try {
			...
			System.out.println("\n\n***********Below packages updated successfully through auto update with global manifest version as:"+GlobalVariables.globalMan+" **********\n");
	...
	public void i_should_validate_installStatus_API_response_wrt_global_manifest() throws Throwable {
		try {
			...
			while(i<jArr.length())
			{
					if(jArr.getJSONObject(i).getString("endpointID").equals(GlobalVariables.endpointIDValue))
					{
						break;
					}
					else
			...
			if(i==jArr.length())
			{
				Assert.fail("AMS installation Status not found");
			}
			element=i;
			Assert.assertEquals(jArr.getJSONObject(element).getString("version"),GlobalVariables.globalMan);
			...
			Assert.assertEquals(jArr.getJSONObject(element).getString("endpointID"),GlobalVariables.endpointIDValue);
			Assert.assertEquals(jArr.getJSONObject(element).getString("agentID"),GlobalVariables.endpointIDValue);
			Assert.assertEquals(jArr.getJSONObject(element).getJSONArray("packageStatus").length(),GlobalVariables.pkgList);
			
			GlobalVariables.pkgMap = new TreeMap<String,String>();
			
			for(int count=0;count<GlobalVariables.pkgList;count++)
			{
				
				JSONObject pObj= jArr.getJSONObject(element).getJSONArray("packageStatus").getJSONObject(count);
				GlobalVariables.pkgMap.put(pObj.getString("name").concat(".exe"),pObj.getString("version"));
	...
	@Then("^User validate installtion variables sent by custom installer and tmp directory details$")
	public void I_validate_installtion_variables_sent_by_custom_installer() throws Throwable {
		try {
			...
			element=i;
			System.out.println("\n Installation parameters sent by custom installer...");
			for(int cu=0; cu<jArr.getJSONObject(element).getJSONArray("packageStatus").length(); cu++)
			{
				
				if(jArr.getJSONObject(element).getJSONArray("packageStatus").getJSONObject(cu).getString("name").equalsIgnoreCase("platform-custom-plugin"))
				{
					System.out.println("\n lmi: "+jArr.getJSONObject(element).getJSONArray("packageStatus").getJSONObject(cu).getJSONObject("installationVariables").getString("lmi"));
					System.out.println("\n webroot: "+jArr.getJSONObject(element).getJSONArray("packageStatus").getJSONObject(cu).getJSONObject("installationVariables").getString("webroot"));
					Assert.assertNotNull(jArr.getJSONObject(element).getJSONArray("packageStatus").getJSONObject(cu).getJSONObject("installationVariables").getString("lmi"), "Failed to send installation variables!!");
					Assert.assertNotNull(jArr.getJSONObject(element).getJSONArray("packageStatus").getJSONObject(cu).getJSONObject("installationVariables").getString("webroot"), "Failed to send installation variables!!");
				}
...
//			Assert.assertEquals(status, "OK");
	...
	@Then("^User validate operation flag in manifest file for feature packages$")
	public void I_validate_operation_flag_in_manifest_file_for_feature_packages() throws Throwable {
		try {
			List<String> platformPackages = new ArrayList<String>(); 
					platformPackages.add("platform-agent-core");
					platformPackages.add("platform-installation-manager");
					platformPackages.add("platform-version-plugin");
					platformPackages.add("platform-asset-plugin");
					platformPackages.add("platform-performance-plugin");
					platformPackages.add("platform-eventlog-plugin");
					platformPackages.add("platform-sysevents-plugin");
					platformPackages.add("platform-systemstate-plugin");
					platformPackages.add("platform-scripting-plugin");
					platformPackages.add("platform-webroot-plugin");
			
			GlobalVariables.scenario.write("\n\nFormatted Response is :" + GlobalVariables.response.replaceAll("\\s+",""));
			JSONObject responseObj= new JSONObject(response);
			String fpkg[] =ems.fp.getPackages();
			for(int j=0;j< fpkg.length;j++) {
				System.out.println("\n Validating feature package : "+fpkg[j] + " for operation flag in manifest file");
				
				for(int i=0; i<responseObj.getJSONArray("packages").length();i++)
				{
					if(responseObj.getJSONArray("packages").getJSONObject(i).getString("name").equalsIgnoreCase(fpkg[j]))
					{
						Assert.assertEquals(responseObj.getJSONArray("packages").getJSONObject(i).getString("operation"),"install");
						System.out.println("\n Validation of feature package : "+fpkg[j] + " is successful with operation flag as " +responseObj.getJSONArray("packages").getJSONObject(i).getString("operation"));
					}
					else if(platformPackages.contains(responseObj.getJSONArray("packages").getJSONObject(i).getString("name")))
					{
						Assert.assertEquals(responseObj.getJSONArray("packages").getJSONObject(i).getString("operation"),"install");
					}		
					else	
						Assert.assertEquals(responseObj.getJSONArray("packages").getJSONObject(i).getString("operation"),"uninstall");	
				}
			}
...
public void i_should_fetch_response_for_Performace_API_ICE() throws Throwable {
	...
	@Then("^I compare the siteIdValue for the above endpoint$")
...
//			Assert.assertEquals(status, "Bad Request");
	...
	@Given("^I read GET response from a micro-service as \"([^\"]*)\"$")
	...
	@Given("^I read GET response from a performance micro-service as \"([^\"]*)\"$")
	public void i__read_GET_response_from_a_performance_micro_service_as(String endpoint) throws Throwable {
		try {

			String url = Utilities.getMavenProperties("Performance-Service-ELB") + endpoint;
			GlobalVariables.scenario.write("I send a Get request for URL " + url);
			GlobalVariables.conn = HTTPUtility.sendGetRequest(url);
		} catch (Exception e) {
			ErrorReporter.reportError(e);
	...
	@Given("^I read GET response from a systemstate micro-service as \"([^\"]*)\"$")
	public void i__read_GET_response_from_a_systemstate_micro_service_as(String endpoint) throws Throwable {
		try {

			String url = Utilities.getMavenProperties("SystemState-Service-ELB") + endpoint;
			GlobalVariables.scenario.write("I send a Get request for URL " + url);
			GlobalVariables.conn = HTTPUtility.sendGetRequest(url);
		} catch (Exception e) {
			ErrorReporter.reportError(e);
	...
	@And("^I clears the Agent Core Log File from tag$")
