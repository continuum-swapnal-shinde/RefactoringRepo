Feature: Agent Auto Update Feature
  ...
  @PlatformRegression1334 @Win7_32Bit @Win7_64Bit @Win8_32Bit @Win8_64Bit @Win10_32Bit @Win10_64Bit @Win8.1_32Bit @WinXP_32Bit @WinXP_64Bit @WinVista_32Bit @WinVista_64Bit @WinServer8_64Bit @WinServer12_64Bit @WinServer16_64Bit @WinServer12R2_64Bit
  ...
  Scenario Outline: Validate agent core to download the individual packages based on global manifest-C3685611
    ...
    Examples: 
      ...
      |        1 | /agent/v1/manifest  | /agent/v1/manifest/gman/partnerManifest |
  ...
  Scenario Outline: Validate auto update functionality E2E-C3790901
    Given SSH connection establishes with remote test machine from tag
    When User read data from excel with rowIndex "<RowIndex>"
    Then User fetch EndPointID of the machine from tag
    ...
    And User validate if system ready for autoupdate feature
    When User send a Post request for Manifest API endpoint as "<JenkinsPOSTEndpoint>"
    Then User should validate the "201" as in response
    When User send a Post request for Manifest API endpoint as "<RRTPOSTEndpoint>"
    Then User should validate the "200" as in response
  ...
  @platformSanity @PlatformRegression @Win7_64Bit
  Scenario Outline: Validate auto update functionality E2E wrt 3rd party package and custom installer-C3813506
    Given SSH connection establishes with remote test machine from tag
    When User read data from excel with rowIndex "<RowIndex>"
    Then User fetch EndPointID of the machine from tag
    When User fetch version Information from the system
    ...
    When User read GET response from a micro-service and store in file with GET as "<GETENDPointPerVersion>"
    Then User should validate the status code as "200"OK
    When User should fetch response for API
    Then User should validate installStatus API response wrt global manifest
    And User validate installtion variables sent by custom installer and tmp directory details
   

    Examples: 
      | RowIndex | JenkinsPOSTEndpoint | RRTPOSTEndpoint           | GETENDPointPerVersion                                       | 
      |        1 | /agent/v1/manifest  | /agent/v1/manifest/gman/partnerManifest | /agent/v1/partner/RandomPartnerID/version/gmf/installStatus | 
